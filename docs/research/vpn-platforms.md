...
## Protocols not yet implemented (honest status)

| Product | Why not in v0.1 | Path forward |
|---------|-----------------|--------------|
| Fortinet FortiClient | CLI varies by version/OS; no single RFC | Research `forticlient` CLI per [FortiClient docs](https://docs.fortinet.com/product/forticlient) |
| Palo Alto GlobalProtect | `gpctl` on macOS; limited Linux | Document per [PAN-OS admin guide](https://docs.paloaltonetworks.com/globalprotect) |
| Pulse Secure / Ivanti | Proprietary | Community CLI scraping only with cited version |

Until adapters exist, set `"vpn_type": "generic"` and rely on universal probes.
