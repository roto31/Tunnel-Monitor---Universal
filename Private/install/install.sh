...
SRC_PLIST="${PAYLOAD_DIR}/LaunchDaemons/com.ruter.tunnel-monitor.plist"
...
UDR7_HOST_DEFAULT="192.168.1.1"
UDR7_USER_DEFAULT="root"
...
show_help() {
    cat <<'EOF'
install.sh — installer for the Mac Studio tunnel monitor
...
WHAT IT DOES
    ...
    3. Generate + authorize SSH key on the UDR7 (if missing)
    4. Install + load /Library/LaunchDaemons/com.ruter.tunnel-monitor.plist
...
install_file "${SRC_OPT}/ssh-udr7-state.sh"  "${INSTALL_DIR}/ssh-udr7-state.sh"  0750
...
# -----------------------------------------------------------------------------
# Phase 3 — SSH key for UDR7 dedup
# -----------------------------------------------------------------------------

step "Phase 3 — SSH key for UDR7 dedup"
...
if [[ -f "${SSH_KEY}" ]]; then
    ...
    yellow "Generating new ed25519 key for UDR7 dedup SSH..."
    ssh-keygen -t ed25519 -f "${SSH_KEY}" -N "" -C "tunnel-monitor@studio" >/dev/null
...
# Determine UDR7 host from config.env if present, else default.
UDR7_HOST="${UDR7_HOST_DEFAULT}"
UDR7_USER="${UDR7_USER_DEFAULT}"
if [[ -f "${INSTALL_DIR}/config.env" ]]; then
    # shellcheck disable=SC1090
    source "${INSTALL_DIR}/config.env" >/dev/null 2>&1 || true
    UDR7_HOST="${UDR7_HOST:-${UDR7_HOST_DEFAULT}}"
    UDR7_USER="${UDR7_USER:-${UDR7_USER_DEFAULT}}"
fi
...
test_udr7_key() {
    ssh -o BatchMode=yes \
        ...
        -i "${SSH_KEY}" \
        "${UDR7_USER}@${UDR7_HOST}" 'echo OK' 2>/dev/null | grep -q '^OK$'
}

if test_udr7_key; then
    green "SSH key already authorized on ${UDR7_USER}@${UDR7_HOST}."
else
    yellow ""
    yellow "==> SSH key needs to be authorized on the UDR7."
    yellow "    You will be prompted for the UDR7 root password ONCE."
    yellow ""
    if ! cat "${SSH_PUB}" | ssh \
            -o StrictHostKeyChecking=accept-new \
            -o UserKnownHostsFile="${SSH_DIR}/known_hosts" \
    ...
    then
        red "ERROR: failed to push pubkey to ${UDR7_USER}@${UDR7_HOST}."
        red "       You can do it manually with:"
        red "         ssh-copy-id -i ${SSH_PUB} ${UDR7_USER}@${UDR7_HOST}"
    ...
    if test_udr7_key; then
        ...
        red "       Check ${UDR7_USER}@${UDR7_HOST}:~/.ssh/authorized_keys manually."
        exit 1
    fi
fi
