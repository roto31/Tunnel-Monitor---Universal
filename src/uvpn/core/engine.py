...
class MonitorEngine:
    ...
    def run_check(self) -> CheckSnapshot:
        ...
        adapter_impl = get_adapter(vpn_type)
        adapter = adapter_impl.probe(cfg_dict)
        stats = adapter_impl.collect_statistics(cfg_dict, adapter)
        logs = adapter_impl.collect_logs(cfg_dict)
        adapter.statistics = stats
        adapter.logs = logs[:50]
