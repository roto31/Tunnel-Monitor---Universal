...
Diagrams use Mermaid. See also [[Network-Overview]] and
[[Getting-Started]].
...
- `state.json` on the Mac is the single source of truth for the SwiftBar
  plugin and `tunnel-check` — neither runs its own checks.
...
Generic guide: [[Spoke-Monitoring]].
